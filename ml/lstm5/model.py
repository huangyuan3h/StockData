...
def get_lstm5_model(n_steps=60, n_features=23, lstm_layer_size=2048):
    model = Sequential([
        ...
        layers.LSTM(int(lstm_layer_size / 2 ** 2), return_sequences=False, name="lstm3"),
        layers.BatchNormalization(),
        layers.Dropout(0.1),
        layers.Dense(int(lstm_layer_size / 2 ** 3), activation='relu', name="dense1"),
        layers.BatchNormalization(),
        layers.Dropout(0.1),
        layers.Dense(int(lstm_layer_size / 2 ** 4), activation='relu', name="dense2"),
        layers.BatchNormalization(),
        layers.Dropout(0.1),
        layers.Dense(int(lstm_layer_size / 2 ** 5), activation='relu', name="dense3"),
        layers.Dense(1),
    ])
